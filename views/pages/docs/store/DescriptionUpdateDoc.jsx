...
export default function DescriptionUpdateDoc() {
    return (
        <div className="space-y-3">
            <h1 className="text-3xl font-bold">Updating A Store's Description</h1>
            <br />
            <p>
                Sometimes, a store's description might need to be updated to reflect new information or changes in the
                store. This documentation covers the API endpoint to help you achieve just that. With this endpoint, you
                can seamlessly update the store's description as needed.
            </p>
            <blockquote>
            ...
            <div className="code-block">
                <h3>Endpoint URL</h3>
                <pre>{`https://estoresapi.com/api/v1/stores/update/description`}</pre>
            </div>
            <div className="code-block">
...
async function changeStoreDescription(newDescription) {
    try {
        const response = await fetch(\`https://estoresapi.com/api/v1/stores/update/description?storeId=\$\{STORE_ID\}\`, {
            method: "POST",
            body: newDescription,
            headers: {
        ...
        else alert("Store description updated successfully!");

        const updatedStore = (await response.json()).data;
    } catch {
        console.log("Error while changing store description", error);
    }
}

changeStoreDescription("New store description");`}
                </pre>
            </div>
            <br />
    ...
    message: "Expected request body to be of type 'text/plain', got type <Incorrect_Content-Type> instead."
            ...
            <div className="flex justify-between items-center max-w-screen-md mx-auto">
                ...
                >
                    <CgArrowLeft /> Updating a store's name
                </Link>
                <Link
                    to="../stores/update/extras"
                    className="flex items-center gap-2 hover:text-blue-600 hover:underline"
                >
                    Adding custom data to store <CgArrowRight />
                </Link>
            </div>
        </div>
    );
